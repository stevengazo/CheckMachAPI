...
{
    options.AddPolicy("MixedCors", builder =>
    {
        builder
            .AllowAnyMethod()
            .AllowAnyHeader()
            .AllowCredentials() // solo si realmente lo necesitas (si no, quitarlo)
            .WithOrigins(
                "https://maquinas.stevengazo.co.cr"
            )
            .SetIsOriginAllowed(origin => true); // permite ANY
    });
});
